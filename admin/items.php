...
if ($conn->connect_error) {
  die("Connection failed: " . $conn->connect_error);
   ...
   if(isset($_POST["submit"])) {
      ...
      $sql = "INSERT INTO items (`name`, `category`,`price`,`gst`)
    VALUES ('$name','$category','$price','$gst')";
                                ...
                                <div class="row">
                                     <div class="col-md-3">
                                        <div class="form-group">
                                            ...
                                            <input class="form-control" type="text" name="iname" id="iname" required>
                                        </div>
                                    </div>
                                    <div class="col-md-3">
                                        <div class="form-group">
                                            <label>Category</label>
                                            <select class="form-control" name="cname" style="border: solid 1px black; border-radius: 10px; border-bottom:solid 2px purple">
                                    ...
                                    <div class="col-md-3">
                                        <div class="form-group">
                                            <label>GST</label>
                                            <input class="form-control" type="number" name="gst" id="gst" required>
                                        </div>
                                    </div>
                                  </div>
                                 
                                        
</div><button type="submit" class="btn btn-info pull-right" name="submit">SAVE </button>
                                    ...
                                    <tbody>
                                         ...
                                         $sr = 0;
                                          ...
                                          <tr>
                                            ...
                                            <button class="btn btn-info" onclick="if(!confirm('Are You Sure Want To Update Profile')){return false;}else{UpdateAgent('<?php echo $result['id'];?>', '<?php echo $result['name'];?>', '<?php echo $result['mobile'];?>', '<?php echo $result['password'];?>', '<?php echo $result['post'];?>');}" type="submit">Update</button></td>
                                          </tr>
                                          <?php  }?>
                                    </tbody>
                                </table>
        ...
        function DeleteAgent(id)
        {
            $('#defaultLoading').modal('show'); 
               $.ajax({
                    ...
                    method:"POST",
                    data:{id:id,request:'DeleteAgent'},
        ...
        function ChangeStatus(id, status)
        {
            $('#defaultLoading').modal('show'); 
               $.ajax({
                    url:"deleteoperation.php",
                    method:"POST",
                    data:{id:id, status:status,request:'ChangeStatus'},
                    success:function(data)
                    {
                        $('#defaultLoading').modal('hide'); 
            ...
            $("#district").change(function()
             ...
             var district=document.getElementById("district").value;
              ...
              $.ajax({
                url:"GetSubdata.php",
                method:"POST",
                data:{district:district,request:'DataCity'},
                success:function(data)
                {
